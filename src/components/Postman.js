...
function Postman({
  ...
  return (
    <div className={`cf ${className} relative`}>
      ...
      {isEditing && !submitted && (
        <div className={showAvatar ? 'left mt05 ml2' : 'left mt05'}>
          <button
            className='light'
            onClick={() => {
              if (!deleteSure) {
          ...
          >
            {!deleteSure ? 'Delete' : 'Deleting! Are you sure?'}
          </button>
        </div>
      )}
      {!submitted && text !== null && (
        <div className='right mt05'>
          <button className='brand cancel' onClick={() => (reset(), onCancel())} disabled={textareaClassName === ''}>
            Cancel
          </button>
          {handler.add && (
            <button className='brand cta' onClick={() => comment('add')} disabled={disableInputs}>
              Comment
            </button>
          )}
          {handler.edit && (
            <button className='brand cta' onClick={() => comment('edit')} disabled={disableInputs}>
              Edit
            </button>
          )}
          {!isEditing && handler.addToReview && (
            <button className='brand cta' onClick={() => comment('addToReview')} disabled={disableInputs}>
              Add review comment
            </button>
          )}
          {!isEditing && handler.addSingleComment && (
            <button className='brand cta' onClick={() => comment('addSingleComment')} disabled={disableInputs}>
              Add single comment
            </button>
          )}
          {!isEditing && handler.startReview && (
            <button className='brand cta' onClick={() => comment('startReview')} disabled={disableInputs}>
              Start review
            </button>
          )}
        </div>
      ...
      {children}
      <Suggestions visible={suggestionsEnabled} onSelect={addToText} />
    </div>
  );
}
