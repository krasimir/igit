...
function File({
  ...
  const items = getFileLines(diffItem, path, toComment, threads, showComments, pr, repo, openComment, postman);
    ...
    <div className='hunk'>
      ...
      <div className='header relative'>
        ...
        <button onClick={getFullFile} className='right'>
          {fullFileContent.state === 'idle' ? (
            <MAXIMIZE size={16} />
          ) : fullFileContent.state === 'loading' ? (
            <MORE_HORIZONTAL size={16} />
          ) : null}
        </button>
                    ...
                    {item.rows.map((row, j) => (
                      <tr className={`code-line ${row.diffLine === 0 ? 'code-line-start' : ''} ${row.type}`} key={j}>
                        <td>
                          <button
                            className='as-link'
                            onClick={() => openComment({ path: row.path, line: row.line, diffLine: row.diffLine })}
                          >
